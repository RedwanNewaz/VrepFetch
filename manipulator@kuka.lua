...
function manipulator:new(o)    
    ...
    self.__index = self
    for i=1,7,1 do
        self.jh[i]=sim.getObjectHandle('j'..i)
    end
...
function manipulator:run()
    ...
    local c=self:findSeveralCollisionFreeConfigsAndCheckApproach(m,300,60)
    ...
    print(txt)
    local path=self:findShortestPath(self:getConfig(),c,6)
    --displayInfo(nil)
